...
static void linux_socket_thread(void *dev_data)
{
	uint8_t i, timeout = 1;
	int status;
	uint8_t data[127*3];
...
static int linux_socket_connect(void *dev_data)
{
	uint8_t i, timeout = 1;
	int status;
	uint8_t data[127*3];
...
static PANCSTATUS linux_init_func(void *dev_data)
{
	char *ipstr = (char *)dev_data;
    #ifdef _WIN32
	win_thread = CreateThread(NULL, 0, &linux_socket_thread, dev_data, 0, NULL);
	#else // Linux
	pthread_create (&my_thread, NULL, (void *) &linux_socket_thread, dev_data);
	#endif

	if (strcmp(ipstr, "::") == 0) {
		return PANCSTATUS_OK;
	}
	clientfd = linux_socket_connect(dev_data);
